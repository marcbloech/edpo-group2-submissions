...
class EventPublisher:
    ...
    def publish(self, event_type: str, data: dict, traceid: str | None = None,
                event_time: str | None = None) -> dict:
        """Publish an event to the worldpulse Kafka topic.

        The message envelope matches the Java Message<T> class:
        - type, id, source, time, data, datacontenttype, specversion (CloudEvents core)
        - traceid, correlationid, group (WorldPulse extensions)

        Args:
            event_type: Event type string (e.g., "MarketAlertEvent")
            data: Event payload dictionary
            traceid: Optional trace ID for end-to-end tracking (auto-generated if None)
            event_time: Optional ISO-8601 timestamp override for the CloudEvents "time"
                field. Used by trailing flush events to advance Kafka Streams' stream
                time past suppress window boundaries.
        ...
        message = {
            ...
            "time": event_time or datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),
            "data": data,
